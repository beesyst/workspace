...
| `Ctrl + R` | Интерактивный поиск по истории | `Ctrl+R` → набери часть команды → `Enter` |
| `fc` | Открывает последнюю команду в $EDITOR для правки и запуска | `fc` → правишь → сохраняешь → команда выполняется |
| `!$` | Подставляет последний аргумент предыдущей команды | `touch notes.md` → `vim !$` откроет `vim notes.md` |
| `grep pattern file` | Не трать конвейер зря: лучше, чем `cat file \| grep pattern` | `grep ERROR /var/log/app.log` |
...
| `cd -` | Быстрый возврат в **предыдущую** директорию |
| `pwd` | Показать полный путь текущей директории |
| `mkdir` | Создать каталог |
| `pushd /some/path` | Перейти и **запомнить** путь (кладет в стек директорий) |
| `popd` | Вернуться к предыдущему пути из стека |
| `dirs -v` | Показать стек директорий с индексами |
| `tree -L 2` | Показать структуру папок в виде дерева (глубина 2) |
